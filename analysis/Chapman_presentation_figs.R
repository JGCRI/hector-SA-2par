## This script makes the figures for the 2019 Chapman Presntation, they are basically the same 
## as the CMS paper plots with a few modifications to improve the visual quality of the plots 
...
BASE_DIR   <- getwd()
OUTPUT_DIR <- file.path(BASE_DIR, 'analysis', 'Chapman_plots')
dir.create(OUTPUT_DIR)

# Consistent plotting scheme 
UNIVERSAL_THEME <- theme_bw(base_size = 24) +   theme(legend.title = element_blank()) 
...
ggplot(run_count_df) + 
  ...
  geom_text(aes(x = filter_name, y = count, label = count), vjust = -0.5, size = 10) +
  UNIVERSAL_THEME + 
...
ggsave(cliamte_bivariate_plot, file = file.path(OUTPUT_DIR, '3B.climate_bivariate_plot.png'), width = 10, height = 5) 
ggsave(carbon_bivariate_plot, file = file.path(OUTPUT_DIR, '3C.carbon_bivariate_plot.png'), width = 10, height = 5) 
...
### 5. CO2 prices ----------------------------------------------------------------------------------------
CO2_price_plot1 <- layered_ribbon_spaghetti(input = policy_rslts$`CO2 prices` %>% 
                           filter(year >= 2010 & filter_name == 'Tgav'), p = 'RF-2p6', title = expression('CO'[2]~' Price'), ylab =  expression('2015$'/'tCO'[2]), x = 'Year') + 
  scale_x_continuous(breaks = seq(from = 1640, to = 2100, by = 10)) + 
  theme(axis.text.x =  element_text(size = 14)) +
  labs(title = NULL) 

CO2_price_plot2 <- layered_ribbon_spaghetti(input = policy_rslts$`CO2 prices` %>% 
                                             filter(year >= 2010), p = 'RF-2p6', title = expression('CO'[2]~' Price'), ylab =  expression('2015$'/'tCO'[2]), x = 'Year') + 
  scale_x_continuous(breaks = seq(from = 1640, to = 2100, by = 10)) +
  theme(axis.text.x =  element_text(size = 14)) +
  labs(title = NULL) 

ggsave(CO2_price_plot1, file = file.path(OUTPUT_DIR, '5A.Global_CO2_price1.png'), width = PRINT_WIDTH, height = PRINT_HEIGHT)
ggsave(CO2_price_plot2, file = file.path(OUTPUT_DIR, '5A.Global_CO2_price2.png'), width = PRINT_WIDTH, height = PRINT_HEIGHT)
...
Global_co2_emiss_plot1 <- layered_ribbon_spaghetti(input = global_co2_emissions_df %>%  filter(filter_name == 'Tgav'), p = 'RF-2p6', 
...
Global_co2_emiss_plot2 <- layered_ribbon_spaghetti(input = global_co2_emissions_df, p = 'RF-2p6', 
                                                   title = expression('Global CO'[2]~' Emissions'), ylab = expression("MTCO"[2]), x = 'Year') + 
  scale_x_continuous(breaks = seq(from = 1640, to = 2100, by = 10)) + 
  theme(axis.text.x = element_text(size = 14)) + 
  geom_hline(aes(yintercept = 0)) + 
...
ggsave(Global_co2_emiss_plot1, file = file.path(OUTPUT_DIR, '6A.Global_CO2_emiss1.png'), width = PRINT_WIDTH, height = PRINT_HEIGHT)
ggsave(Global_co2_emiss_plot2, file = file.path(OUTPUT_DIR, '6A.Global_CO2_emiss2.png'), width = PRINT_WIDTH, height = PRINT_HEIGHT)
...
global_rf1 <- layered_ribbon_spaghetti(input = policy_rslts$`Climate forcing` %>% filter(filter_name == "Tgav"),
...
global_rf2 <- layered_ribbon_spaghetti(input = policy_rslts$`Climate forcing`,
                                       p = 'RF-2p6', title = expression('Climate Forcing'), 
                                       ylab =  expression('W/m'^2), x = 'Year') + 
  scale_x_continuous(breaks = seq(from = 1640, to = 2100, by = 10)) + 
  theme(axis.text.x =  element_text(size = 14)) +
...
ggsave(global_rf1, file = file.path(OUTPUT_DIR, '9A.Global_RF1.png'), width = PRINT_WIDTH, height = PRINT_HEIGHT)
ggsave(global_rf2, file = file.path(OUTPUT_DIR, '9A.Global_RF2.png'), width = PRINT_WIDTH, height = PRINT_HEIGHT)
