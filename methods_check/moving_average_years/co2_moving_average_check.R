...
# right now we are strating with just the co2 data. 
...
# filter_flag.csv
yrs15_filter_flag        <- read_csv(file.path(data_dir, "filter_flag_15years.csv"))
yrs15_filter_flag$window <- '15'

yrs7_filter_flag        <- read_csv(file.path(data_dir, "filter_flag_7years.csv"))
yrs7_filter_flag$window <- '7'

filter_flag <- bind_rows(yrs15_filter_flag, yrs7_filter_flag)

# obs
yrs15_obs        <- read_csv(file.path(data_dir, "G.NOAA_CO2_observation_ma_15years.csv"))
yrs15_obs$window <- '15'

yrs7_obs        <- read_csv(file.path(data_dir, "G.NOAA_CO2_observation_ma_7years.csv"))
yrs7_obs$window <- '7'
...
# Hector 
yrs15_hector        <- read_csv(file.path(data_dir, "G.co2_hector_ma_15years.csv"))
yrs15_hector$window <- '15'

yrs7_hector        <- read_csv(file.path(data_dir, "G.co2_hector_ma_7years.csv"))
...
selected_rslt <-  filter(hector, run_name %in% select_runs)

# Plot the observation ribbon and the hector runs against one another. 
ggplot() + 
  geom_ribbon(data = observations, aes(year, ymin = min_value, ymax = max_value, fill = window), alpha = 0.4) -> 
  obs_plot
...
save(script_output, file = file.path(data_dir, "co2_figures.rda"))

# Conclusion --- no not really
