...
file_name <- file.path( BASE, 'sub-out', 'A.Hector_GCAM_parameters_boxplot_selection.csv' )
write.csv( Hector_GCAM_parameters, file = file_name, row.names = FALSE )

file_name <- file.path(BASE, 'sub-out', 'A.Hector_GCAM_parameters_mapping_boxplot_selection.csv')
write.csv( Hector_GCAM_parameter_mapping, file = file_name, row.names = FALSE )
save(script_output, file = file.path(BASE, 'diag-out', 'Tgav_2100_boxplot_selection.rda'))
