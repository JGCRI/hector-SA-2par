...
NPP_Dn_input %>% 
  ...
  distinct %>% 
  Dc_func(alpha = 0.05) %>% 
  mutate(variable = 'Tgav, NPP multi') -> 
  NPP_Tgav_Dc

NPP_Tgav_Dmetric_results <- join_Dmetric(NPP_Tgav_Dc, NPP_Dmetric_results)
...
write.csv(Dmetric_results, file = file.path(OUTPUT_DIR, 'E.Dnmetric_results.csv'), row.names = FALSE)
