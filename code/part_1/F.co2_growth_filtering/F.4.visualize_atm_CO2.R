# Purpose: Visulaize the moving averages are used to filter Hector runs by the 
# atmospheric CO2.
...
visulaize_obs_hector <- function(hector_wide, observations, title = NULL, sample_number = 25){
  ...
  ggplot() + 
    ...
    geom_ribbon(data = observations, aes(year, ymin = growth_min, ymax = growth_max, fill = "Observations"), alpha = 0.3) + 
...
observations <- readr::read_csv(file.path(BASE, "int-out", "observations", "F.CDIAC_growth_ma.csv"))


# Import and format the data frame in preperation for plotting.
list.files(file.path(BASE, 'int-out'), 'F.growth_hector_ma.csv', full.names = T, recursive = T) %>% 
  lapply(., function(X){readr::read_csv(X) %>% mutate(rcp = X)}) %>% 
  bind_rows %>% 
  filter(variable == "atmos_c") %>% 
  # Add the rcp name
  mutate(rcp = gsub("/F.growth_hector_ma.csv", "", rcp)) %>% 
...
# 2. Save ------------------------------------------------------------------
save(output, file = file.path(output_path, "F.plots.rda"))
message("Figures saved at ", file.path(output_path, "F.plots.rda"))
